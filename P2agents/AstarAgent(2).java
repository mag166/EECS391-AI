...
public class AstarAgent extends Agent {
    ...
    class MapLocation
    {
        ...
        public MapLocation(int x, int y, MapLocation origin, float cost)
        {
            this.x = x;
            this.y = y;
    ...
    private Stack<MapLocation> AstarSearch(MapLocation start, MapLocation goal, int xExtent, int yExtent, MapLocation enemyFootmanLoc, Set<MapLocation> resourceLocations)
    {
        ...
        MapLocation currentNode = null;
        //Closed list
        ArrayList<MapLocation> closed = new ArrayList<MapLocation>();
        PriorityQueue<MapLocation> open = new PriorityQueue<MapLocation>();
        boolean completed = false;
        start.cost = 0;
        start.cheby = chebyshev(start,goal);
        ...
        while(open.peek() != null)
        {
            ...
            for (int x = -1; x < 2; x++) {
                for (int y = -1; y < 2; y++) { // iterate over all nodes around current
                    if (currentNode.x + x >= 0 && currentNode.x + x < xExtent && currentNode.y + y >= 0 && currentNode.y + y < yExtent &&(x != 0 || y != 0)) { // check if it's on the map
                        //this will be the location we are trying
                        MapLocation test = new MapLocation(currentNode.x + x, currentNode.y + y);
                        
                        //see if it is a resource location or an enemy footman
                        if (resourceLocations.contains(test) == false && test.equals(enemyFootmanLoc) == false) {
                            neighbors.add(test);
                        }
                    }
                }
            ...
            for(MapLocation n : neighbors){
                ...
                n.cheby = chebyshev(n, goal);
                
                if(closed.contains(n)){
                    continue;
                }
                
                if(currentNode.cost + 1 > n.cost){
                    continue;
                }
